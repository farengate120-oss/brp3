# Script for generating some of documents in `dev-docs` from BenjForum's code
import ast
import os
...
PLUGIN_MANIFEST = "misago.plugins.manifest.BenjForumPlugin"
...
def generate_hooks_reference_index(hooks_data: dict[str, dict[str, ast.Module]]):
    ...
    with open(PLUGINS_HOOKS_PATH / "reference.md", "w") as fp:
        ...
        fp.write("\n\n")
        fp.write(
            "This document contains a list of all standard plugin hooks existing in BenjForum."
        )
        fp.write("\n\n")
...
def generate_outlets_reference():
    ...
    with open(PLUGINS_PATH / "template-outlets-reference.md", "w") as fp:
        ...
        fp.write("\n\n")
        fp.write(
            "This document contains a list of all built-in template outlets in BenjForum."
        )
